...
void free_self_from_net()
{
    ...
    int mulch = one_chance_in(5);
    dprf("net.net_durability: %d, ATTR_HELD: %d",
         hold, you.attribute[ATTR_HELD]);

    // Escaping from a net takes 2-5 turns
    you.attribute[ATTR_HELD] -= 1 + coinflip() + coinflip();
    if (you.attribute[ATTR_HELD] < 1)
    {
        if (mulch)
            destroy_item(net);
        else
